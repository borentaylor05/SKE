class ArcCheckTracker < ActiveRecord::Base
	...
	scope :search_numeric, -> (search) { where("cast(check_num as varchar) like ? or cast(amount as varchar) like ? or cast(case_id as varchar) like ? or order_num like ? or crs like ?", "%#{search.to_s}%", "%#{search.to_s}%", "%#{search.to_s}%", "%#{search.to_s}%", "%#{search.to_s}%" )}
