...
#	validates :case_id, presence: true
	...
	scope :search_numeric, -> (search) { where("cast(check_num as varchar) like ? or cast(case_id as varchar) like ? or order_num like ? or crs like ? or check_amount like ?", "%#{search.to_s}%", "%#{search.to_s}%", "%#{search.to_s}%", "%#{search.to_s}%", "%#{search.to_s}%" )}
